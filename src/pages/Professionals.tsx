  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-extrabold tracking-tight text-white">Profissionais</h1>
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {filtered.map(pro => {
            ...
            return (
              <div key={pro.id} className="bg-[#21262d] rounded-xl p-5 border border-white/[0.08] hover:border-blue-600/50 hover:shadow-black/40 transition-all">
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-semibold text-slate-100 text-sm">{pro.name}</p>
                  ...
                  <button
                    ...
                    className={`flex items-center gap-1 text-xs px-2 py-1 rounded-full font-medium transition-colors ${
                      pro.status === 'active'
                        ? 'bg-green-500/[0.1] text-green-400 hover:bg-green-500/[0.18]'
                        : 'bg-[#0d1117] text-slate-500 hover:bg-slate-200'
                    }`}
                  >
                ...
                <div className="grid grid-cols-3 gap-2 mb-4 p-3 bg-[#161b22] rounded-xl">
                  <div className="text-center">
                    <p className="text-xs text-slate-400">Tarefas</p>
                    <p className="text-sm font-bold text-slate-200">{taskCount}</p>
                  </div>
        ...
        <div className="fixed inset-0 bg-slate-900/60 flex items-center justify-center z-50 p-4">
          <div className="bg-[#21262d] rounded-xl border border-white/[0.08] shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 bg-[#21262d] flex items-center justify-between px-6 py-4 border-b border-white/[0.05] z-10">
              ...
              <button onClick={() => setShowModal(false)} className="text-slate-400 hover:text-slate-500 p-1 rounded-lg hover:bg-white/[0.06] transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="p-6 space-y-5">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {TASK_TYPES.map(type => (
                    <div key={type} className="flex items-center gap-2 p-2 bg-[#161b22] rounded-lg">
                      <label className="text-xs text-slate-500 w-24 flex-shrink-0 font-medium">{getTaskTypeLabel(type)}</label>
                      <input
                        type="number"
                        min="0"
                        step="0.01"
                        ...
                        className="flex-1 border border-white/[0.08] rounded-lg px-2 py-1.5 text-xs focus:outline-none focus:ring-2 focus:ring-blue-500 bg-[#21262d]"
                        placeholder="0,00"
                      />
                    </div>
                  ))}
            ...
            <div className="sticky bottom-0 bg-[#21262d] flex gap-3 px-6 py-4 border-t border-white/[0.05]">
              <button
                ...
                className="flex-1 border border-white/[0.08] text-slate-500 py-2.5 rounded-lg text-sm font-medium hover:bg-white/[0.04] transition-colors"
              >
                Cancelar
              </button>
              <button
      ...
      {showViewModal && viewingPro && (
        <div className="fixed inset-0 bg-slate-900/60 flex items-center justify-center z-50 p-4">
          <div className="bg-[#21262d] rounded-xl border border-white/[0.08] shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 bg-[#21262d] flex items-center justify-between px-6 py-4 border-b border-white/[0.05] z-10">
              <h2 className="text-lg font-bold text-slate-100">Perfil do Profissional</h2>
            ...
            <div className="p-6 space-y-6">
              <div className="flex items-center gap-5">
                ...
                <div>
                  <h3 className="text-xl font-bold text-slate-100">{viewingPro.name}</h3>
                  ...
                  <span className={`mt-1 inline-flex text-xs px-2.5 py-1 rounded-full font-semibold ${
                    viewingPro.status === 'active' ? 'bg-green-500/[0.1] text-green-400' : 'bg-white/[0.08] text-slate-400'
                  }`}>
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                <div className="bg-green-500/[0.1] rounded-xl p-4 text-center border border-green-500/[0.2]">
                ...
                <div className="bg-blue-500/[0.1] rounded-xl p-4 text-center border border-blue-100">
              ...
              <div className="space-y-2">
                <h4 className="text-sm font-semibold text-slate-200">Informações de contato</h4>
                <div className="flex items-center gap-2 text-sm text-slate-500"><Mail className="w-4 h-4 text-slate-400" />{viewingPro.email}</div>
                <div className="flex items-center gap-2 text-sm text-slate-500"><Phone className="w-4 h-4 text-slate-400" />{viewingPro.phone || 'Não informado'}</div>
                <div className="flex items-center gap-2 text-sm text-slate-500"><Key className="w-4 h-4 text-slate-400" />{viewingPro.pixKey || 'Chave Pix não cadastrada'}</div>
              </div>

              <div>
                <div className="flex items-center justify-between mb-3">
                  <h4 className="text-sm font-semibold text-slate-200">Tarefas ({viewingDemands.length})</h4>
                </div>
                {viewingDemands.length === 0 ? (
                  ...
                  <div className="space-y-2 max-h-40 overflow-y-auto">
                    {viewingDemands.slice(0, 8).map(d => (
                      <div key={d.id} className="flex items-center justify-between text-xs p-2 bg-[#161b22] rounded-lg">
              ...
              <div>
                <h4 className="text-sm font-semibold text-slate-200 mb-3">Histórico financeiro</h4>
                  ...
                  <div className="divide-y divide-white/[0.05]">
                    {viewingMovements.map(m => (
                      <div key={m.id} className="py-2.5 flex items-center justify-between">
                        <div>
                          <p className="text-sm font-medium text-slate-200">{m.demandTitle}</p>
                          <p className="text-xs text-slate-400">{formatDate(m.completedAt)} · {m.clientName}</p>
                        </div>
                        <div className="text-right ml-4">
                          <p className="text-sm font-bold text-slate-100">{formatCurrency(m.value)}</p>
      ...
      {deleteId && (
        <div className="fixed inset-0 bg-slate-900/60 flex items-center justify-center z-50 p-4">
          <div className="bg-[#21262d] rounded-xl border border-white/[0.08] shadow-xl w-full max-w-sm p-6">
            ...
            <div className="flex gap-3">
              <button onClick={() => setDeleteId(null)} className="flex-1 border border-white/[0.08] text-slate-500 py-2.5 rounded-lg text-sm font-medium hover:bg-white/[0.04]">
                Cancelar
              </button>
